...
print(f"mpi4py : [{MPI.COMM_WORLD.Get_rank()}/{MPI.COMM_WORLD.Get_size()}]\n")
...
if __name__ == "__main__":
    ...
    with ( scalapack(b'C', 2, 2) as context
        ...
        for il in range(a.local_m):
            for jl in range(a.local_n):
                if not a.data[il,jl] == a0d.data[il,jl]:
                    print(f"elpa[{myrank}/{nranks}] a[{il},{jl}] : {a.data[il,jl]} =? {a0d.data[il,jl]} : {a.data[il,jl] == a0d.data[il,jl]}")
                    raise ValueError
    ...
    eigenvectors = DistributedMatrix.like(a)
    ...
    # for Elpa local and global indices are 0-based
    # DistributedMatrix also has indxl2g and indexg2l
    # which are also 0-based, compliant with common Python behavior.
    for il in range(a.na_rows):
        for jl in range(a.na_cols):
            i,j = a.get_global_index(il,jl)
            # print(f"elpa[{myrank}/{nranks}] ({il},{jl}) -> ({i},{j}) ")
            a.data[il,jl] = aij(i+1,j+1)

    print(f"elpa[{myrank}/{nranks}] a \n{a.data}")
    ...
    if myrank == 0:
        print("*** elpa.eigenvectors solution ***")
        print("Eigenvalues:")
        for i in range(na):
            print(f"{i} {eigenvalues[i]}")
        norm_diff = np.linalg.norm(eigenvalues-eigenvalues0)
