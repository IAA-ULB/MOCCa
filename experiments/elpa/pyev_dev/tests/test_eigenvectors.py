...
@pytest.mark.parametrize("na,nev,nblk", parameter_list_all)
def test_all_eigenvectors(na, nev, nblk):
    ...
    comm = MPI.COMM_WORLD
    header = f"{inspect.currentframe().f_code.co_name}({na=},{nev=},{nblk=})-{comm2str(comm)}"
    if comm.Get_rank() == 0:
...
@pytest.mark.parametrize("na,nev,nblk", parameter_list_part)
def test_part_eigenvectors(na,nev,nblk):
    """
    pyscalapack: must use pdsyevr instead of pdsyev
    pyelpa: computes all eigenvalues and part of the eigenvectors. which part is unclear to me...
    """
    pass
...
if __name__ == "__main__":
    the_test_you_want_to_debug = test_all_eigenvectors
