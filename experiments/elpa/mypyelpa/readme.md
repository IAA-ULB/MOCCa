...
Robin attended me to the fact that ELPA includes a wrapper! It takes a while to build, but works. i added `--enable-python` and `--enable-python-tests` to the script in `builingElpa/build4vaughan-intel.sh` and that went fine.

So all this work was eventually for nothing - but i learned a few things about `ctypes`. 
