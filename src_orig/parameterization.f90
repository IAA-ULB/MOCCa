...
$PARAMDECL
    ...
    !---------------------------------------------------------------------------    
    ! Special flag to enable backwards compatibility with an earlier, bugged 
    ! version of Tantalus. If you set  this to .true., the charge form factor
    ! of the neutron will not be taken into account correctly into the 
    ! single-particle hamiltonian. This was the way the original GSk1 and GSk2
    ! parameterizations were fitted, and this is retained here for reproducing
    ! those calculations.
...
contains
   ...
   subroutine readparameterization(name_param, func_name) 
    ...
    &              Estabp, Estabn, neutroncoulomberror,                        &
