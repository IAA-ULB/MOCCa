...
module parameterization
 ...
 ! BCASTPARAMS= [too long to be copied]
...
$PARAMDECL
    ...
    !---------------------------------------------------------------------------
    ! Treatment options for the corrections for spurious collective motion
    ! (i) the movement of the center of mass
    !   (a) the one-body part
    !       COM1BODY
    !       --------
    !       0: not included
    !       1: perturbatively included
    !       2: included self-consistently
    integer       :: COM1body = 2
    !   (b) the two-body part
    !       COM1BODY
    !       --------
    ...
    !       rotcorr
    !       --------
    !       0: no rotational correction included in the mean-field calculation
    !       1: rotational correction based on the Belyaev moment of inertia
    integer       :: RotCorr  = 0
    ! (iii) vibrational motion
    !       vibcorr
    !       --------
    !       0: no vibrational correction included in the mean-field calculation
    !       1: vibrational correction based on the Belyaev moment of inertia
    !          for rotational motion
    !       2: vibrational correction based on the Belyaev moment of inertia
    !          associated with the quadrupole deformation
    integer       :: Vibcorr   = 0
    !---------------------------------------------------------------------------
    ! Further options for the rotational correction when rotcorr = 1
    ! See G. Scamps et al., Eur. Phys. J. A (2021) 57:333
    ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ! Parameters
    real(KIND=dp) :: RotcorrB    = 0, rotcorrC = 0
    ! Whether or not to use a cutoff in the definition of the rotational correction.
    ...
    !---------------------------------------------------------------------------
    ! Further options for the rotational correction when vibcorr = 1
    ! See W. Ryssens et al., Eur. Phys. J. A (2022) 58:246
    ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
contains
   ...
   subroutine readparameterization(name_param, func_name) 
    ...
    ! Only the very first MPI rank does the reading 
    if(MPI_RANK.eq.0) then
      ...
      ! - forces.param
      ! If none of these options work, stop.
...
$CHECKPARAMS
...
#if(USE_MPI > 0)
    !---------------------------------------------------------------------------
    ! Broadcasting all relevant variables
    ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -   
    ! a) generic parameterization options
    call MPI_BCAST(hbm      , 2, MPI_REAL8  , 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(COM1body , 1, MPI_INTEGER, 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(COM2body , 1, MPI_INTEGER, 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(eps      , 1, MPI_REAL8  , 0, MPI_COMM_WORLD, mpi_err)
    ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -   
    ! b) Coulomb options
    call MPI_BCAST(coultreatment      , 1, MPI_INTEGER, 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(coulorder          , 1, MPI_INTEGER, 0, MPI_COMM_WORLD, mpi_err)
    ...
    call MPI_BCAST(nucleonsize_selfconsistent, 1, MPI_LOGICAL, 0, &
    &                                                MPI_COMM_WORLD, mpi_err)
    ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -   
    ! c) collective correction options
    call MPI_BCAST(rotcorr      , 1, MPI_INTEGER, 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(vibcorr      , 1, MPI_INTEGER, 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(rotcorrb     , 1, MPI_REAL8  , 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(rotcorrc     , 1, MPI_REAL8  , 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(rotcutwindow , 2, MPI_REAL8  , 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(rotcutmu     , 2, MPI_REAL8  , 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(vibcorrb     , 1, MPI_REAL8  , 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(vibcorrl     , 1, MPI_REAL8  , 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(vibcorrd     , 1, MPI_REAL8  , 0, MPI_COMM_WORLD, mpi_err)

    call MPI_BCAST(hocomform    , 1, MPI_LOGICAL, 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(rotcorr_cut  , 1, MPI_LOGICAL, 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(nucleonsize_selfconsistent, 1, MPI_LOGICAL, 0, &
    &                                                MPI_COMM_WORLD, mpi_err)
    ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -   
    ! d) pairing options
    call MPI_BCAST(Estabp      , 1, MPI_REAL8  , 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(Estabn      , 1, MPI_REAL8  , 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(cutneutron  , 1, MPI_REAL8  , 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(cutproton   , 1, MPI_REAL8  , 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(pairingmu   , 1, MPI_REAL8  , 0, MPI_COMM_WORLD, mpi_err)
    call MPI_BCAST(cuttype     , 1, MPI_integer, 0, MPI_COMM_WORLD, mpi_err)
    ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -   
    ! e) parameters of the Skyrme EDF
    !    Note: these broadcast calls are auto-generated by Hephaestos  
$BCASTPARAMS
    ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -   
    ! Nobody moves until everybody has the correct parameters ...
    call MPI_barrier(MPI_COMM_WORLD, mpi_err)
#endif
    
    !---------------------------------------------------------------------------
    ! Bookkeeping to be done by all MPI_ranks
    ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ! a) setting the rotcutwindow and rotcorrmu parameters
    !    (when not read from the .param file)
...
$RESETPARAMS 
  ...
  subroutine printparameterization(param_name, func_name)
    ...
    88 format ('   Gaussian form factors substituted by special folding functions.')
    ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -    
