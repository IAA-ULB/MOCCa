...
contains 
  ...
  &                        gradnorm, expectedDE, r, k, gauge, maxiter)
    ...
    real(KIND=dp), intent(in)    :: h(:,:), gaps(:,:), alpha(2), r(:,:), k(:,:)
    ...
    do iter=1,maxiter
        ...
        expectedDE = expectedDE - alpha(1) * sum((H20 - lambda * N20)**2)
...
$NTR  Bogo(sb  +1:sb  +T, sb+1:sb+T) = Bogo(sb+T+1:sb+2*T, sb+T+1:sb+2*T)
  ...
  &                            particles, targetN, alpha, blocks)
    ...
    real(KIND=dp), intent(in)    :: alpha(2)
  ...
  &                            particles, targetN, alpha, blocks)
    ...
    if(abs(N).lt.pairing_prec .or. all(alpha .eq. 0.0d0)) then
      bogo = nbA
      return
    endif
...
!     
  ...
  function GradUpdate(grad, bogo, alpha, blocks) result(newbogo)
    ...
    do B=1,4,2
      ...
      &                   -alpha((B+1)/2) *matmul(tV,grad(si+1:si+T,si+1:si+T)) 

      ! V = V - alpha U gradient
      newbogo(sb+T+1:sb+2*T,sb+T+1:sb+2*T)=newbogo(sb+T+1:sb+2*T,sb+T+1:sb+2*T)&
$NTR      &               -alpha((B+1)/2) *matmul(tU,grad(si+1:si+T,si+1:si+T)) 
$TR       &               +alpha((B+1)/2) *matmul(tU,grad(si+1:si+T,si+1:si+T)) 
