...
$NOTAU    else
$NOTAU      Kinetic = CompKinetic_density()
$NOTAU    endif
    ! COM correction
...
$NOTAU  function CompKinetic_density() result(kinetic)
$NOTAU    !---------------------------------------------------------------------------
...
$TR   COM2_pp_debug = 2*COM2_pp_debug
  ...
  subroutine calcFields(calcall, precon)
    ...
    real(KIND=dp)              :: radius
$PRECON_ACTIVE    real(KIND=dp), allocatable :: update(:,:)
    ...
    if(simulate_spherical_bc) then 
      ...
      do i=1,mv 
        ...
        if(radius .gt. (nx + 0.5d0) * dx) then 
          F_I_I(i,1) = F_I_I(i,1) + 1000.0d0
          F_I_I(i,2) = F_I_I(i,2) + 1000.0d0
          F_I_I(i,3) = F_I_I(i,1) + F_I_I(i,2)
          F_I_I(i,4) = F_I_I(i,1) - F_I_I(i,2)
        endif
      enddo
    endif
    !---------------------------------------------------------------------------
