module functional
 ...
 implicit none
    ...
    ! c) Potentials
    ! d) Previous_values of the potentials
...
$TR   COM2pp = 2*COM2pp 
  ...
  subroutine calcPotentials(calcall)
    !---------------------------------------------------------------------------
    ! Calculate all of the Skyrme and both Coulomb potentials based on the 
    ! densities currently in memory.
    ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ! Input:
    !  calcall : logical that determines whether all potentials are forcefully 
    !            recalculated from the densities.  
    !            If calcall is true, all of the potentials get recalculated.
    !            If calcall is false, only potentials that vanish identically
    !            get recalculated.
    !---------------------------------------------------------------------------
