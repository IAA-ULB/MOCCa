...
contains
   ...
   subroutine create_blocked_list_canonical(blocked_list_canonical)
      !-----------------------------------------------------------------------------
      ! Identify the single-particle states in the canonical basis that look like
      !  they are blocked. We attempt this by
      !
      !  1. Identifying a candidate canonically conjugate partner \bar{i} by
      !     looking for large values of
      !         < \psi_i | T | \psi_{\bar{i}} >
      !
      !  2. and checking whether \rho(i,i) - \rho(ibar,ibar)  is large
      !      in the canonical basis.
      !
      ! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ! Output:
      ! blocked_list_canonical : 1D array of size (nwt)
      !                          .true.  if the state is "blocked"
      !                          .false. if the state is not
      !-----------------------------------------------------------------------------
      ...
      do B = 1, 8, 2
         ...
         do wave = 1, NB
            do wave_TR = 1, NB_TR
               ! Calculates overlap between a given spwf and another from the block
