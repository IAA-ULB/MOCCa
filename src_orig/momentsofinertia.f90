...
contains
   ...
   subroutine calcJ2andBelyaev_HFB
      ...
      use wavefunctions, only: jz, jz_no_cut, jx_no_cut, jx, jy_no_cut, jy
      ...
      !---------------------------------------------------------------------------
      ! Angular momentum calculated in the QP basis WITH pairing cutoff
      real(KIND=dp) :: J20(nwt, nwt, 3), J11(nwt, nwt, 3), cut_cr
      ! ... and also without pairing cutoff
      ...
      if (.not. (allocated(jz))) then
         allocate (jx(nwt, nwt), jx_no_cut(nwt, nwt))
         allocate (jy(nwt, nwt), jy_no_cut(nwt, nwt))
