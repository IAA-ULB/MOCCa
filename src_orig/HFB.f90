...
$TR    endif
    ...
    if(blocktype .ne. 4) then
      ...
      call figure_out_blocking_structure_EFA( Bogo,  &
...
$PBROKEN blockblock(i) = 5
  ...
  subroutine figure_out_blocking_structure_EFA(&
  &          Bogo, config,bl_qps, part_qps, p_overlaps, BlockType, blocklowest) 
    !-----------------------------------------------------------------------------
    !
    !
    !
...
!    5 format (' Grad. norm      = ',  4es9.2)
  ...
  &                                             rhotransfo, kappatransfo, ifail, Bogoliubov)
    !---------------------------------------------------------------------------
    ! a) Diagonalize  Rho
    ...
    real(KIND=dp), intent(in)  :: kappa_pairing(nwt,nwt), bogoliubov(2*nwt,2*nwt)
    real(KIND=dp), intent(out) :: rho_can(nwt), kappa_can(nwt)
    real(KIND=dp), intent(out) :: rhotransfo(nwt,nwt), kappatransfo(nwt,nwt)
 
    integer, intent(out) :: ifail
   
    real(KIND=dp), allocatable :: tmp(:,:), work(:), temp_occ(:), R(:,:), R2(:,:)
    real(KIND=dp), allocatable :: complete_bogo(:,:)
    
    integer, allocatable       :: indices(:)
    integer :: si, N, N2, B, i,j, k, lwork, effN, ind, ii, jj, T, sb
...
!      if(B.eq.1) then
!        do i=1,N
!          print ('(99f18.6)'), rho_pairing(i,1:N)
!        enddo
!        print *
!      endif
