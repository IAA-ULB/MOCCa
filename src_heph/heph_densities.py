...
def GenDensityExpression(denin,derivative_combinations,intermediate, 
    ...
    if((not so.timelike) or leftwave != rightwave or TimeDen(density) == +1):
        ...
        # Start the expression generation
        ...
        # Generate all of the combinations of indices, both contracted and uncontracted
        args_dict = build_index_combinations(LeftOperator.dimension + RightOperator.dimension, ndim, coupling, cross)

        for arg in args_dict.keys(): # <--- loop over the uncontracted indices 
            IND = ''
            for mu in arg:
                IND = IND + ',' + str(int(abs(mu) + 1))  # Python indexes 0:N-1

            # Build the indices for the lhs of the expression
            dic['IND'] = IND
            if len(dic['IND']) > 1:
            ...
            for true_arg in args_dict[arg]:  # <--- loop over the full set of indices, including contracted ones
                # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
