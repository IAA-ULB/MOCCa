    ...
    for den in src_heph.heph_functional.Densities_needed:
        ...
        for term in src_heph.heph_functional.Functional_terms:
            ...
            for test in densities:
                derj, lapj, leftj, rightj, coupj, crossj = ParseOperators(
                    test, so.timelike
                )
        ...
        for nterm, term in enumerate(src_heph.heph_functional.Functional_terms):
            ...
            for c in cpl:
                for i in range(len(densities)):
                    if OrderOfDen(densities[i]) != OrderOfDen(
                        densities[i], contract=False
                    ):
                ...
                for i, altden in enumerate(densities):
                    ...
                    if (
                        ...
                        if dden == "1" or i != 0:
                            ...
                            lap_combinations = itertools.product(
                                indiv_lap, repeat=altlap
                            )
                            ...
                            for lcmb in lap_combinations:
                                ...
                                der_combinations = itertools.product(
                                    range(nden), repeat=altder
                                )
                                ...
                                for dcmb in der_combinations:
                                    ...
                                    if dcmb.count(0) == 1 and dden != "1":
                                        if len(dcmb) != 1:
                                            print(
                                                "Hephaestos cannot yet deal with derivatives upon derivatives of density dependent terms!"
                                            )
                                            exit(1)
                                        ...
                                        if len(lcmb) > 1:
                                            print(
                                                "Repeated derivatives of density dependence."
                                            )
                                            exit(1)
                                    ...
                                    fieldlist[iso].append(
                                        [
                                            a,  # all the densities in the contribution to the field
        ...
        if "P" not in den:
            # Only recombine normal fields
            fieldread_hdf5 = fieldread_hdf5 + ts.field_transfo_recomb_hdf5.substitute(
                dic
            )
        ...
        FIELDCALC_perturbed = FIELDCALC_perturbed + ts.field_calc_a_start.substitute(
            dic
        )

        FIELDCALC = FIELDCALC + ts.field_condition_start.substitute(dic)
        FIELDCALC_perturbed = FIELDCALC_perturbed + ts.field_condition_start.substitute(
            dic
        )
        ...
        for iso in ["0", "1", "n", "p"]:
            ...
            FIELDCALC_perturbed = (
                FIELDCALC_perturbed + ts.field_calc_iso_start.substitute(dic)
            )
            for fieldterm in fieldlist[iso]:
                ...
                NumberOfIndices, nthree, densities, newcpl, pisigns = Adaptdensities(
                    fieldterm[0], fieldterm[4], fieldterm[1], fieldterm[2]
                )
                ...
                for arg in true_args:
                    ...
                    if (
                        LeftOperator.derorder + RightOperator.derorder == 2
                    ...
                    for i, d in enumerate(densities):
                        ...
                        if fieldterm[+6] != "1" and i == 0:
                            ...
                            dic["EXPR1"] = dic["EXPR1"] + ts.field_calc_DD.substitute(
                                dic
                            )
                        else:
                            # Ordinary expression
                            dic["EXPR1"] = dic["EXPR1"] + ts.field_calc_den.substitute(
                                dic
                            )
                    ...
                    for j in range(len(densities)):
                        ...
                        for i, d in enumerate(densities):
                            ...
                            if fieldterm[+6] != "1" and i == 0:
                                ...
                                if i != j:
                                    dic["EXPR_PERT"] = dic[
                                        "EXPR_PERT"
                                    ] + ts.field_calc_DD.substitute(dic)
                                else:
                                    dic["EXPR_PERT"] = dic[
                                        "EXPR_PERT"
                                    ] + ts.field_calc_DD_pert.substitute(dic)
                            else:
                                # Ordinary expression
                                if i != j:
                                    dic["EXPR_PERT"] = dic[
                                        "EXPR_PERT"
                                    ] + ts.field_calc_den.substitute(dic)
                                else:
                                    dic["EXPR_PERT"] = dic[
                                        "EXPR_PERT"
                                        ] + ts.field_calc_den_pert.substitute(dic)
                        ...
                        FIELDCALC_perturbed = (
                            FIELDCALC_perturbed + ts.field_calc_pert.substitute(dic)
                        )
        ...
        if "P" not in den:
            ...
            FIELDCALC_perturbed = (
                FIELDCALC_perturbed + ts.field_recombination.substitute(dic)
            )

        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
        # DIRTY HACK 
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
        #  Multiply the pairing potentials by a global factor 1/2 because of the 
        #  subtleties of the coexistence of \tilde{D}^{I,I} and \tilde{D}^{1,1,*}
        #  that Hephaestos cannot handle.
        #
        # NOTE: this ONLY works if the pairing channel has terms with the form 
        #           
        #   \tilde{D}^{A,B} \tilde{D}^{A,B,*}   or \tilde{C}^{A,B} \tilde{C}^{A,B,*}
        # 
        # i.e. terms that are bilinear in the SAME local pairing density.
        # 
        if "P" in den: 
            FIELDCALC = FIELDCALC + ts.field_calc_fac2.substitute(dic)
            FIELDCALC_perturbed = FIELDCALC_perturbed + ts.field_calc_fac2.substitute(dic)
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 

        FIELDCALC = FIELDCALC + ts.field_condition_end.substitute(dic)
        FIELDCALC = FIELDCALC + ts.field_line.substitute(dic) + "\n"

        FIELDCALC_perturbed = FIELDCALC_perturbed + ts.field_condition_end.substitute(
            dic
        )
        FIELDCALC_perturbed = FIELDCALC_perturbed + ts.field_line.substitute(dic) + "\n"
            ...
            for arg in args:
                ...
                if len(coupling) + len(cross) == 0:
                    ...
                    for c in fullcont:
                        ...
                        ii = 0
                        for i in range(
                            LeftOperator.dimension + RightOperator.dimension
                        ):
                            found = False
                            ...
                            for combination in cross:
                                if i == combination[0]:
                                    p = p + (
                                        c[cross.index(combination) + len(coupling)],
                                    )
                                    found = True
                                if i == combination[1]:
                                    p = p + (
                                        c[cross.index(combination) + len(coupling) + 1],
                                    )
                                    found = True
                            if not found:
                                p = p + (arg[ii],)
                ...
                if extder != 0:
                    ...
                    px, py, pz = AxisReflection(
                        LeftOperator, RightOperator, larg, rarg, so, "P" in den
                    )
                    dic["PX"] = str(px)
                    dic["PY"] = str(py)
                    dic["PZ"] = str(pz)
                    ...
                    dic["DIVISOR"] = (
                        extder // 2
                    ) ** 2  # ansatz for the appropriate preconfactor
                    fieldprecon = fieldprecon + +(
                        extder // 2
                    ) * ts.field_precon_call.substitute(dic)
...
def GenerateAction(field, symmetrize, so):
    ...
    for true_larg in largs:
        ...
        # HACK 
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        # Include a factor of two that arises in the calculation of matrix elements 
        # \Delta because Hephaestos cannot deal with the subtleties of the coexistence 
        # of \tilde{D}^{A,B} and \tilde{D}^{A,B,*} coexisting
        if('P' in field):
            dic["LMULT"] = dic["LMULT"] + " 2.0d0 * "
                    ...
                    for l in range(LeftOperator.dimension):
                        ...
                        for c in coupling:
                            if l in c:
                                Found = True
                        if not Found:
                            dic["FIELDIND"] = dic["FIELDIND"] + ",%d" % int(
                                abs(true_larg[l]) + 1
                            )
            ...
            if Found:
                ...
                for k in range(4):
                    ...
                    for l in range(lorder):
                        dic["RCOMP"] = (
                            str(true_larg[-l - offset - 1] + 1) + "," + dic["RCOMP"]
                        )
