subroutine calc_F_I_I(D_I_I, Lap_D_I_I, D_Nm_Nm, Der_C_I_NS, coupl_constant, sigma, sigma_b, $TOFILL, ngrid, DP_I_I)
  !--------------------------------------------------------------------------------------------
  ...
  F_I_I(:,1) = F_I_I(:,1)  &
       & + coupl_constant(3) * (sigma)  * pow(D_I_I(:,1), (sigma - 1)) * D_I_I(:,1) * D_I_I(:,1)

  F_I_I(:,1) = F_I_I(:,1)  &
       & + coupl_constant( 3)  * pow(D_I_I(:,1), sigma) * D_I_I(:,1)

  F_I_I(:,1) = F_I_I(:,1)  &
       & + coupl_constant( 3)  * pow(D_I_I(:,1), sigma) * D_I_I(:,1)

  F_I_I(:,1) = F_I_I(:,1)  &
       & + coupl_constant(4) * (sigma)  * pow(D_I_I(:,1), (sigma - 1)) * D_I_I(:,2) * D_I_I(:,2)

  F_I_I(:,1) = F_I_I(:,1)  &
       & + coupl_constant(5) * (sigma_b)  * pow(D_I_I(:,1), (sigma_b - 1)) * D_I_I(:,1) * D_I_I(:,1)

  F_I_I(:,1) = F_I_I(:,1)  &
       & + coupl_constant( 5)  * pow(D_I_I(:,1), sigma_b) * D_I_I(:,1)

  F_I_I(:,1) = F_I_I(:,1)  &
       & + coupl_constant( 5)  * pow(D_I_I(:,1), sigma_b) * D_I_I(:,1)

  F_I_I(:,1) = F_I_I(:,1)  &
       & + coupl_constant(6) * (sigma_b)  * pow(D_I_I(:,1), (sigma_b - 1)) * D_I_I(:,2) * D_I_I(:,2)
  ...
  F_I_I(:,2) = F_I_I(:,2)  &
       & + coupl_constant( 4)  * pow(D_I_I(:,1), sigma) * D_I_I(:,2)

  F_I_I(:,2) = F_I_I(:,2)  &
       & + coupl_constant( 4)  * pow(D_I_I(:,1), sigma) * D_I_I(:,2)

  F_I_I(:,2) = F_I_I(:,2)  &
       & + coupl_constant( 6)  * pow(D_I_I(:,1), sigma_b) * D_I_I(:,2)
