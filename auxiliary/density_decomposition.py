...
def decompose_density_axial(fname, rr, ls=[0], power=5, dcolumn=4):
  """
    ...
    not depend on phi, to simplify the calculations. Moreover, in its current
    implementation, this routine assumes that the symmetry axis of the nuclear
    density is the Z-axis. If this needs changing, one should adapt the 
    definition of the polar coordinates in the "integrand" routine below.
    ...
    Input:
      ...
      power  : the power of 2 for which to take integration points along the 
               theta direction. More specifically, this routine takes
                    2**power + 1 equally spaced points
...
def decompose_density(fname, rr, ls=[0], power=5, dcolumn=4):
  """
    ----------------------------------------------------------------------------
    Obtain the multipole components of an (axially symmetric) density, i.e.    
    get some of the coefficients in 
    
    rho(r,theta,phi) = sum_{l=0}^{\infty} sum_{m=0}^{l}  r_lm(r)  Y_lm(theta,phi)

    by explicitly integrating 
      
    r_lm(r) = int dtheta dphi sin (theta) r(theta, phi).
    
    Note that we do not take the 
