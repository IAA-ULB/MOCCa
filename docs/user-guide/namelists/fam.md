...
| **External field**      |           |             |                                                              |
| `l`                     | integer   | 0           | Multipolarity of the external field                          |
| `m`                     | integer   | 0           | Projection (orientation) of the external field               |
...
Some further remarks of general interest:
 ...
 1. When passing `omega_min` `omega_max` and `omega_step`, FAM will be solved for a range of frequencies omega = `omega_min`  + k * `omega_step` < `omega_max` for k=0,...
 2. The external field is proportional to the solid harmonics f_lm = r^l Y_lm, except for the monopole operator which defined as f_00 = r^2. 
 3. By setting `eff_charge_n` = `eff_charge_p`, the external field corresponds to an isoscalar operator, while setting `eff_charge_n` = - `eff_charge_p` defines an isovector perturbation. 
