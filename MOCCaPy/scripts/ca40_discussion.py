...
from mocca.mfstates import SlaterDeterminant
...
# dim=3 is default
wf = SlaterDeterminant(Z=20, N=20, initializer='Nilsson', mesh=mesh, nwp=40, nwn=40)
param = Param("BSkG1")
bxl = param.create_EDF()

mfe_solver = MFESolver(energy_precision=1e-9, moment_precision=1e-3, wf0=wf, strategy='heavy_ball', scf='linear_mix', edf=bxl)
...
E = calculate_energy(wf) # wf may store (a reference) to bxl
# you might conceivably want to calculate the energy wf with another model > specify kwarg edf=...
Espwf = calculate_spwf_energy(wf, potentials) # alternate way to calculate the energy; potentials are required
