...
def test_differentiate_3D_Laplacian(check=True, debug=True):
    """Verify that differentiation yields the same result on reduced and non-reduced grids."""
    M, d = 32, 0.8
    mesh  = LagrangeMesh(M=M, d=d, reduced=False)
    meshR = LagrangeMesh(M=M, d=d, reduced=True)

    n_neutrons, n_protons, nwn, nwp = 20, 20, 15, 15
    osc_freq = (0.2, 0.2, 0.2)
    hfpsi = HFPsi(mesh=mesh,
    ...
    nwt = nwn + nwp
    assert hfpsiR.n_total_wf == nwt
    assert hfpsi .n_total_wf == nwt

    # OOPS even the wave function generated by nilsson is not the same
    # which one is correct? how does nilsson now whether the grid is reduced or not?
    N = M // 2
    for iz in range(N):
        for iy in range(N):
            for ix in range(N):
                for iw in range(nwt):
                    print(f"[{iz},{iy},{ix},{iw}] {hfpsiR.dataG[ix,iy,iz,iw]} {hfpsi.dataG[N+ix,N+iy,N+iz,iw]}")
                    # assert hfpsiR.dataG[ix,iy,iz,iw] == pytest.approx(hfpsi.dataG[N+ix,N+iy,N+iz,iw])