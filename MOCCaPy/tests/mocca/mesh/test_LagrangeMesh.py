...
def test_LagrangeMesh_basis_function_1D():
    ...
    N = 6
    d = 1
    mesh = LagrangeMesh(N=(N,), d=d, reduced=False)
    # r = mesh.gx
    ...
    for i in range(N):
        ...
        plt.legend()
        fig.savefig(project_folder/f"MOCCaPy/tests/mocca/mesh/1D_basis_function_{i}.png")
        fig.clear()
        plt.close(fig)
...
def test_LagrangeMesh_basis_function_2D():
    ...
    N = 6
    d = 1
    mesh = LagrangeMesh(dim=2, N=N, d=d, reduced=False)
    # r = mesh.gx
    nr1 = 201
    ...
    for i in range(N):
        for j in range(N):
            ...
            plt.legend()
            fig.savefig(project_folder/f"MOCCaPy/tests/mocca/mesh/2D_basis_function_{i}_real.png")
            # plt.show()
            plt.close(fig)
            ...
            plt.legend()
            fig.savefig(project_folder/f"MOCCaPy/tests/mocca/mesh/2D_basis_function_{i}_imag.png")
            # plt.show()
            plt.close(fig)
